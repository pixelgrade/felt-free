 ...
 */
function felt_jetpack_setup() {
	// Add theme support for Infinite Scroll.
	add_theme_support( 'infinite-scroll', array(
		'container' => 'posts-container',
		'render'    => 'felt_infinite_scroll_render',
		'footer'    => 'page',
		'footer_widgets' => pixelgrade_is_active_sidebar( 'sidebar-footer' ) || has_nav_menu('footer' ),
		'wrapper'   => false
	) );
	...
	add_theme_support( 'jetpack-content-options', array(
		...
		'post-details'       => array(
			'stylesheet'      => 'felt-style', // name of the theme's stylesheet.
...
add_action( 'after_setup_theme', 'felt_jetpack_setup' );

/**
 * Custom render function for Infinite Scroll.
 */
function felt_infinite_scroll_render() {
	while ( have_posts() ) {
		the_post();
 ...
 * @return array
 */
function felt_default_jetpack_sharing_services( $enabled ) {
	return array(
		'visible' => array(
			'facebook',
			'twitter',
...
add_filter( 'sharing_default_services', 'felt_default_jetpack_sharing_services', 10, 1 );
 ...
 * @return array
 */
function felt_default_jetpack_sharing_options( $default ) {
	$default = array(
		'global' => array(
			'button_style' => 'text',
			'sharing_label' => false,
...
add_filter( 'pixelgrade_filter_jetpack_sharing_default_options', 'felt_default_jetpack_sharing_options', 10, 1 );

/**
 * Prevent sharing buttons when a Featured Posts widget starts.
 */
function felt_remove_jetpack_sharing() {
...
add_action( 'pixelgrade_featured_posts_widget_start', 'felt_remove_jetpack_sharing', 10 );

/**
 * Add sharing logic after a Featured Posts widget has rendered.
 */
function felt_add_jetpack_sharing() {
	if ( function_exists( 'sharing_display' ) ) {
		add_filter( 'the_content', 'sharing_display', 19 );
		add_filter( 'the_excerpt', 'sharing_display', 19 );
	}
}
add_action( 'pixelgrade_featured_posts_widget_end', 'felt_add_jetpack_sharing', 10 );
 ...
 */
function felt_jetpackme_remove_rp() {
...
add_filter( 'wp', 'felt_jetpackme_remove_rp', 20 );

function felt_jetpack_more_related_posts( $options ) {
	$options['size'] = 3;

	return $options;
}
add_filter( 'jetpack_relatedposts_filter_options', 'felt_jetpack_more_related_posts', 10, 1 );
 ...
 * @return array|bool
 */
function felt_get_jetpack_related_posts_ids( $post = null ) {
	$post = get_post( $post );
	...
	if ( class_exists( 'Jetpack_RelatedPosts' ) && method_exists( 'Jetpack_RelatedPosts', 'init_raw' ) ) {
		...
		$related = Jetpack_RelatedPosts::init_raw()
		                               ->set_query_name( 'felt-jetpack-related-posts' ) // Optional, name can be anything
		                               ->get_for_post_id(
				$post->ID,
				array(
 ...
 * @return bool|void
 */
function felt_the_jetpack_related_posts_headline( $default = null ) {
	$headline = '';
 ...
 * @return array|bool
 */
function felt_get_jetpack_related_posts_options() {
	if ( class_exists( 'Jetpack_RelatedPosts' ) && method_exists( 'Jetpack_RelatedPosts', 'init' ) ) {
 ...
 * @return array
 */
function felt_jetpack_related_posts_customize_options( $options ) {
...
add_filter( 'jetpack_related_posts_customize_options', 'felt_jetpack_related_posts_customize_options', 10, 1 );
 ...
 * @return array
 */
function felt_jetpack_top_posts_custom_thumb_size( $get_image_options ) {
	$get_image_options['width'] = 405;
	$get_image_options['height'] = 304;

	return $get_image_options;
}
add_filter( 'jetpack_top_posts_widget_image_options', 'felt_jetpack_top_posts_custom_thumb_size' );
